...
if (isset($_POST['register_submit'])) 
    {
        ...
        $stmt_check = $conn->prepare("SELECT id FROM admin WHERE email = ?");
    ...
    if ($result_check->num_rows > 0) 
    ...
    else 
        {
            ...
            $stmt_insert = $conn->prepare("INSERT INTO admin (name, email, number, gender, password, address) VALUES (?, ?, ?, ?, ?, ?)");
        ...
        if ($stmt_insert->execute()) 
            {
                ...
                header("Location: AdminLogin.php?status=registered");
                exit(); 
            } 
        else 
            {
